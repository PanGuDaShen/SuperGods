...
//        id_main_rv_2
    ...
    override fun setStyleDialog(): Int {
        return R.style.medialog
    }
...
//        })
    ...
    override fun bindData() {
        if (context != null) {
            niceAdapter = NiceAdapter(context, NiceDialogList.getList(),ListenerAdapter())
            id_main_rv_2?.let {
    ...
    inner class ListenerAdapter : NiceHodler.OnNiceClickListener {
        override fun onClicks(str: NiceDialogList.NicedialogDatabean) {
            Toast.makeText(context,"key=${str.key}  veluce = ${str.veluce}",Toast.LENGTH_SHORT).show()
            LogUtils.e("Key = ${str.key}   valuce = ${str.veluce}")
            tabLayout?.let {
                when(str.key){
                    "setDrawOrder" -> {
